...
// Allows %Uptime(format=\"%O:%M:%S.%f\")% to be used in ini config file for
// property Format. boost::log::attributes::timer value type is
// boost::posix_time::time_duration
class UptimeFormatterFactory : public boost::log::basic_formatter_factory<
      ...
      return boost::log::expressions::stream
             << boost::log::expressions::format_date_time<
                    ...
                    boost::log::expressions::attr<
                        boost::posix_time::time_duration>(name),
                    it->second);
    } else {
      return boost::log::expressions::stream
             << boost::log::expressions::attr<boost::posix_time::time_duration>(
                    name);
    }
  }
};

void Logger::init() { initFromConfig(""); }

void Logger::initFromConfig(const std::string &configFileName) {
  // Disable all exceptions
  boost::log::core::get()->set_exception_handler(
      boost::log::make_exception_suppressor());
  ...
  boost::log::register_simple_formatter_factory<
      boost::log::trivial::severity_level, char>("Severity");
  // Allows %TimeStamp(format=\"%Y.%m.%d %H:%M:%S.%f\")% to be used in ini
  // config file for property Format.
  boost::log::register_formatter_factory(
      "TimeStamp", boost::make_shared<TimeStampFormatterFactory>());
  // Allows %Uptime(format=\"%O:%M:%S.%f\")% to be used in ini config file for
  // property Format.
  boost::log::register_formatter_factory(
      "Uptime", boost::make_shared<UptimeFormatterFactory>());
  ...
  if (configFileName.empty()) {
    ...
    if (!ifs.is_open()) {
      ...
      WARN("Unable to open logging config file: " << configFileName);
    } else {
      try {
...
void Logger::disable() { boost::log::core::get()->set_logging_enabled(false); }
...
void Logger::addDataFileLog(const std::string &logFileName) {
  ...
  backend->add_stream(
      boost::shared_ptr<std::ostream>(new std::ofstream(logFileName)));
